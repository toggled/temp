...
from gcnWGTL import gcnWGTL

# def WGTLtrain(model, labels, idx_train,optimizer):
#     model.train()
#     optimizer.zero_grad()
#     output, loss_topo = model(model.features, model.adj_norm, model.global_witness_complex_feat, self.local_witness_complex_feat.to(self.device))
...
def main(args):
    ...
    elif args.backbone == "gcnWGTL":
        if args.ptb_rate == 0:
            global_witness_complex_feat = torch.FloatTensor('data/' + args.dataset + '_'+str(args.ptb_rate)+'_PI' + '.npz', allow_pickle=True)['arr_0'] # torch.FloatTensor(np.random.rand(1, 50, 50))
            local_witness_complex_feat_ = np.load('data/' + args.dataset + '_' + str(args.ptb_rate) + '_localPI' + '.npz', allow_pickle=True)['arr_0']
        else:
            global_witness_complex_feat = torch.FloatTensor(np.load('data/' + args.dataset + '_'+str(args.ptb_rate)+'_PInew' + '.npz', allow_pickle=True)['arr_0']) # torch.FloatTensor(np.random.rand(1, 50, 50))
            local_witness_complex_feat_ = np.load('data/' + args.dataset + '_' + str(args.ptb_rate) + '_localPInew' + '.npz', allow_pickle=True)['arr_0']
    ...
    for run in range(args.runs):
        ...
        if args.backbone == 'gcnWGTL':
            model = gcnWGTL(nfeat=d, nhid=nhid, nclass=c, \
                        dropout=drop_rate, lr=lr, weight_decay=wd, \
                            device=device, alpha = alpha, beta = beta, gamma = gamma, \
                            lambda_coeff = lambda_coeff, aggregation_method = aggregation_method)
            model = model.to(device)
            # print(type(labels))
            # print(labels.shape)
            # [data.adj, perturbed_data.adj][args.perturbed]
            model = model.fit(features, adj_mtx ,\
                                witness_complex_feats[0], witness_complex_feats[1], \
                                labels, verbose=True)
        ...
        for epoch in range(args.epochs):
            ...
            if args.backbone == "gcnWGTL":
                print('loss=train()')
                loss = model._train_without_val(labels, idx_train, optimizer)
                print('eval()')
                model.eval()
                print('result = test()')
                result = model.test(idx_train, idx_val, idx_test)
            else:
