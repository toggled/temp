...
class gcnWGTL(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, dropout=0.5, lr=0.01, weight_decay=5e-4,
        ...
        # self.gc12 = GraphConvolution(nhid, nhid, with_bias=with_bias)
    ...
    def fit(self, features, adj, global_witness_complex_feat, local_witness_complex_feat, labels, initialize=True, verbose=False, normalize=True, **kwargs):
        if initialize:
            self.initialize()
        ...
        # print('total time: ',time()-s_tm)
        return self
    
    def _train_without_val(self, labels, idx_train,optimizer):
        self.train()
    ...
    #     best_loss_val = 100
    #     best_acc_val = 0
    #     print(self.features.get_device())
    ...
    def test(self, idx_train, idx_val, idx_test):
        ...
        train_acc = utils.accuracy(out[idx_train], self.labels[idx_train])
        valid_acc = utils.accuracy(out[idx_val], self.labels[idx_val])
        test_acc = utils.accuracy(out[idx_test], self.labels[idx_test])
